(() => {
  ...
  function showView(name) {
    ...
    document.body.classList.toggle("is-splash", name === "Home");
  ...
  function refreshHome() {
    ...
    const detail = $("splashResumeDetail");
    if (state.finished) {
      $("btnResume").querySelector("span").textContent = "Lihat Ringkasan";
      if (detail) detail.textContent = `${formatMonth(state.meta.month)} • ${count} helaian selesai`;
    } else if (state.currentResult) {
      $("btnResume").querySelector("span").textContent = "Sambung Pengiraan";
      if (detail) detail.textContent = `${formatMonth(state.meta.month)} • Helaian ${state.currentResult.index + 1} telah dikira`;
    } else {
      const next = count + 1;
      $("btnResume").querySelector("span").textContent = "Sambung Pengiraan";
      if (detail) detail.textContent = `${formatMonth(state.meta.month)} • Seterusnya Helaian ${next}`;
    }
  }
